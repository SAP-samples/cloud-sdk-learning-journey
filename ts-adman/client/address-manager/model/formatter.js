...
sap.ui.define([], function () {
  return {
    ...
    creationDateValue: function (sTimestamp) {
      ...
      return i18n.getText(
        'createdOn',
        new Date(sTimestamp).toLocaleString('en-US', {
          weekday: 'long',
          year: 'numeric',
          month: 'long',
          day: 'numeric',
    ...
    masterPageTitle: function (aElements) {
      return this.getResourceBundle().getText('masterPageTitle', [
        aElements.length ? aElements.length : 0,
      ]);
    },

    addressListTitle: function (aAdresses) {
      return this.getResourceBundle().getText('addresses', [
        aAdresses && aAdresses.length ? aAdresses.length : 0,
      ]);
    },
